...
function scene:create( event )
	...
	local background = display.newImageRect("대화창-마을배경/캐릭터대화창-빨강마을배경.png",display.contentWidth, display.contentHeight)
	background.x,background.y = display.contentWidth/2,display.contentHeight/2
	sceneGroup:insert(background)


	local wiz = display.newImage("캐릭터-기본(그림자O)/캐릭터-블리(그림자O).png")
	wiz.x, wiz.y = display.contentWidth/2,display.contentHeight*0.5
	sceneGroup:insert(wiz)
	wiz.alpha = 0


	local happy_wiz = display.newImage("캐릭터-기쁨(그림자O)/캐릭터-블리기쁨(그림자O).png")
	happy_wiz.x, happy_wiz.y = display.contentWidth/2,display.contentHeight*0.5
	sceneGroup:insert(happy_wiz)
	happy_wiz.alpha = 0

	local nangam_wiz = display.newImage("캐릭터-난감(그림자O)/캐릭터-블리난감(그림자O).png")
	nangam_wiz.x, nangam_wiz.y = display.contentWidth/2,display.contentHeight*0.5
	sceneGroup:insert(nangam_wiz)
	nangam_wiz.alpha = 0

	local sad_wiz = display.newImage("캐릭터-슬픔(그림자O)/캐릭터-블리슬픔(그림자O).png")
	...
	if name =="bag/perfume.png" then
		happy_wiz.alpha = 1
		daewha = display.newText("꺄아! 이거 내 생일 선물이야? 내가 좋아하는 달달한 체리향 향수잖아! 세상이 빨간 하트로 차오르는 기분이야.",105,550,"font/NanumSquare_acB.ttf")
    	...
    	nangam_wiz.alpha = 1
    	daewha = display.newText("이거… 내 선물이야? … 그래, 고마워.",105,550,"font/NanumSquare_acB.ttf")
        daewha.anchorX,daewha.anchorY = 0,0
        daewha:setFillColor(0)
        --daewha.alpha = 0
        daewha.size = 25
	...
	local function next2(event)
		...
		if event.phase == "began" then
			composer.removeScene("viewz_bully")
